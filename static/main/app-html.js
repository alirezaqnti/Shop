...
(function ($) {
	...
	THEME.Video = (function () {
		...
		function keepScale(slider) {
			...
			if (
				...
				if (
					...
					var bnrH =
						(parseInt(
							$bnsliderNoScale.attr('data-start-mheight'),
							10
						) /
							parseInt(
					...
					var bnrH =
						(parseInt(
							$bnsliderNoScale.attr('data-start-height'),
							10
						) /
							parseInt(
								$bnsliderNoScale.attr('data-start-width'),
								10
							)) *
						wW;
					if (
						...
						$bnsliderNoScale.closest('.holder.boxed').length
					) {
						var colW =
							($bnsliderNoScale.closest('.container').width() *
								100) /
							wW;
						bnrH = (bnrH * colW) / 100;
					}
		...
		function toggleExpandVideo(playerId, expand) {
			...
			if (expand) {
				if (isMobile()) {
					...
					newHeight =
						video.$videoWrapper.width() / videoOptions.ratio;
				}
		...
		function sizeBackgroundVideo($videoPlayer) {
			...
			if (isMobile()) {
				...
				var desktopHeightMax = Math.ceil(
		...
		function setBackgroundVideo(playerId) {
			...
			videos[playerId].$videoWrapper.addClass(
	...
	THEME.initialization = {
		init: function init() {
			...
			if ($('.fixed-scroll').length)
				this.compensateScrollBar('.fixed-scroll');
			...
			if ($('.js-video-content').length)
				this.videoContent('.js-video-content');
		...
		tabScroll: function tabScroll(accordion) {
			$('.collapse', $(accordion)).on('show.bs.collapse', function (e) {
				...
				if (
					$panel.prevAll().filter($open.closest('.panel')).length !==
					0
				) {
				...
				var MainSlider = {
					...
					_animate: function _animate() {
						var that = this,
							...
							arrowsplace = $wrapper.find(
								that.options.arrowsplace
							),
						...
						function doAnimations(elements, first) {
							if (first) {
								$('.bnslider-first-slide').removeClass(
									'bnslider-first-slide'
								);
								return false;
							}
						...
						$slider.on('init', function (slick) {
							...
							if (isMobile) {
								...
								if (
									!$('.bnslider-image', $currentSlide).find(
										'img'
									).length
								) {
									startInit($firstAnimatingElements);
						...
						$slider.on(
							...
							function (event, slick, currentSlide, nextSlide) {
								...
								var $nextSlide = $slider.find(
										'.bnslider-slide[data-slick-index="' +
											nextSlide +
											'"]'
									),
									$animatingElements =
						...
						function controlVideo(slick, control, button) {
							var video = void 0,
								...
								userConrol = button;
							if (userConrol)
								currentSlide.addClass('userControl');
						...
						function sizeBackgroundVideo($slide) {
							...
							if (slideWidth / videoOptionsRatio < playerHeight) {
								playerWidth = Math.ceil(
									playerHeight * videoOptionsRatio
								);
								...
								playerHeight = Math.ceil(
									slideWidth / videoOptionsRatio
								);
								$player
									.width(slideWidth)
									.height(playerHeight)
		...
		scrollOnLoad: function scrollOnLoad() {
			...
			if ($elem.length) {
				setTimeout(function () {
					var speed =
						$body.height() / 3 > 500 ? $body.height() / 3 : 500;
					var wHeight =
				...
				THEME.simplefilters = {
					...
					init: function init(options) {
						...
						$gallery.each(function () {
							var $gallery = $(this),
								$galleryItem = $(
									that.default.galleryItem,
									$gallery
								),
								$filterLabel = $(
									that.default.filterLabel,
									$gallery
								),
								activeStart = void 0;
							that._handlers(
							...
							if (!activeStart)
								$filterLabel.first().trigger('click');
					...
					if (!isMobile && !$('.aside').length) {
						if ($('.hdr-transparent').length) {
							...
							$(this.default.text).css({
								'padding-top':
									$(this.default.header).outerHeight() * 0.85,
							});
						}
					} else {
						$(this.default.text).css({
		...
		countdown: function countdown(_countdown) {
			...
			$(_countdown).each(function () {
				var $countdown = $(this),
					promoperiod = void 0,
					isActual = false,
					timeLocale = ['روز', 'ساعت', 'دقیقه', 'ثانیه'];
				if ($body.data('time-locale')) {
					timeLocale = $body.data('time-locale').split('/');
				}
				if ($countdown.attr('data-promoperiod')) {
					promoperiod = parseInt(
						$countdown.attr('data-promoperiod'),
						10
					);
				...
				if ($countdown.attr('data-countdown')) {
					...
					isActual =
						Date.parse(promoperiod) - Date.parse(new Date()) > 0;
				}
				if (isActual) {
					$countdown
				...
				if ($countdown.closest('.js-countdown-wrap').length) {
					$countdown
						.closest('.js-countdown-wrap')
						.addClass('countdown-init');
				}
			});
		},
			...
			THEME.flowtype = {
				...
				init: function init(bnr) {
					...
					$(bnr).each(function () {
						...
						$this.imagesLoaded(function () {
							var fontratio =
								Math.round($this.attr('data-fontratio') * 100) /
								100;
							if (fontratio > 0) {
					...
					var $el = $(el),
						...
						fontSize =
							fontBase > this.default.maxFont
								...
								: fontBase;
					setTimeout(function () {
						$el.css('font-size', fontSize + 'px').addClass(
							'fontratio-calc'
						);
					}, 100);
				},
				...
				$('[data-toggle="tooltip"]').on(
					...
					function () {
						if ($('.tooltip').hasClass('bs-tooltip-right')) {
							...
							$('.tooltip').hasClass('bs-tooltip-bottom')
						) {
					...
					function setMaxHeight(wHeight, submenu) {
						...
						if (submenu.length) {
							var maxH = $body.hasClass('has-sticky')
								? wHeight -
								  $('.hdr-content-sticky').outerHeight()
								: wHeight -
					...
					$('li', $submenu)
						...
						.on('mouseleave', function () {
							...
							var $this = $(this)
								...
								.removeClass('hovered');
							$('.submenu-link-image', $(this)).removeClass(
								'to-right'
							);
							if ($('> a .mmenu-preview', $this).length) {
								var $ul = $this.closest('ul');
		...
		mmobilePush: function mmobilePush() {
			THEME.mobilemenupush = {
				...
				_setHeigth: function _setHeigth() {
					$('.nav-wrapper').css({
						height: $(
							'mmenu-submenu-active .nav-level-' +
								(this.defaults.curLevel + 1)
						).outerHeight(),
					});
				},
			...
			THEME.headerdrop = {
				...
				_handlers: function _handlers() {
					var that = this,
						...
						$dropLinkParent = $dropLink.closest(
							that.defaults.dropLinkParent
						),
						...
						if (!$dropLink.data('mclick')) {
							...
							$dropLink.on('click.dropdn', function (e) {
								...
								var $this = $(this),
									$panel = $($this.data('panel')).length
										...
										: $this.next();
								if (
									$this.closest('.js-popupAddToCart').length
								) {
									THEME.selectModal &&
								...
								if ($this.closest('.mobilemenu').length) {
									...
									if (
										$this.parent().hasClass('dropdn_search')
									) {
										setTimeout(function () {
											$this
												.next()
							...
							$dropLinkParent.on('click.dropdn', function (e) {
								...
								if (
									...
									$('.header-side-panel').find('.is-opened')
										.length &&
							...
							$dropLink.on('click.dropdn', function (e) {
								...
								if ($panel.length) {
									...
									if ($this.parent().hasClass('is-hovered')) {
										...
										$this.next().css({
											'max-height':
												that._getDropHeight($this) +
												'px',
											top: that._getDropPos($this) + 'px',
										});
										if (
											$this.closest('.dropdn_fullheight')
												.length
										) {
											$this.next().css({
												height:
							...
							$document.on('click.dropdn', function (e) {
								...
								if (!$this.closest('.dropdn-content').length) {
									...
									if (
										...
										$dropLinkParent.hasClass('is-hovered')
									) {
										$dropLinkParent.removeClass(
											'is-hovered'
										);
				...
				_getDropHeight: function _getDropHeight(dropdn) {
					...
					if (
						$parent.length &&
						dropdn.closest('.dropdn_fullheight,.dropdn_fullwidth')
							.length
					) {
					...
					$this.each(function () {
						...
						if (!isMobile) {
							if (!$('.hdr-content-sticky').length) {
								_.css({
									'max-height':
										that._getDropHeight(_.prev()) + 'px',
									top: that._getDropPos(_.prev()) + 'px',
								});
								if (_.closest('.dropdn_fullheight').length) {
									_.css({
										height:
											that._getDropHeight(_.prev()) +
											'px',
									});
								}
							} else {
				...
				THEME.stickyheader = {
					...
					_multirow: function _multirow() {
						if ($(this.defaults.hdrNav).height() > 60) {
							...
							$(this.defaults.header).removeClass(
								'mmenu-multirow'
							);
					},
					_multirowS: function _multirowS() {
						if ($body.hasClass('has-hdr_sticky')) {
							if ($(this.defaults.stickyNav).height() > 60) {
								$(this.defaults.headerSticky).addClass(
									'mmenu-multirow-s'
								);
							} else
								$(this.defaults.headerSticky).removeClass(
									'mmenu-multirow-s'
								);
						}
					},
					...
					_setScroll: function _setScroll() {
						var that = this,
							...
							checkColission =
								!isMobile ||
								$(that.defaults.stickyCollision).length
									? true
									: false;
						...
						$window.on('scroll.stickyHeader', function () {
							...
							if (checkColission) {
								$(that.defaults.stickyCollision).each(
									function () {
										var $this = $(this),
											h = parseInt(
												$(
													that.defaults.headerSticky
												).outerHeight(),
												10
											);
										if (
	...
	THEME.modals = {
					...
					$.fancybox.open(
						...
						],
						{
							...
							afterClose: function afterClose() {
								...
								if (
									$('.js-form-notification .js-scroll-to')
										.length
								) {
									THEME.initialization.scrollToTarget(
										'.js-scroll-to'
									);
									$(
										'.js-form-notification .js-scroll-to'
					...
					$document.on('click', '.js-prd-quickview', function (e) {
						...
						$.fancybox.open({
							...
							afterShow: function afterShow() {
								...
								setTimeout(function () {
									$('.quickview-gallery').css({
										opacity: '1',
									});
								}, 500);

								if (
									$('.js-product-previews-carousel-qw').length
								) {
									$('.js-product-previews-carousel-qw').slick(
										'setPosition'
									);
								}
								THEME.initialization.countdown(
									'#modalQuickView .js-countdown'
								);
				...
				_setScroll: function _setScroll() {
					if (w > maxMD) {
						if ($('.modal-quickview--classic').length) {
							var h = $('#modalQuickView .quickview-info')[0]
								.scrollHeight;
							$('#modalQuickView .quickview-info').css({
								'min-height': h,
								height: h,
		...
		checkOut: function checkOut() {
			THEME.checkOutModal = {
				...
				setData: function setData(data) {
					...
					if (data.path) {
						if (data.path.indexOf('no-image') == -1) {
							$(this.defaults.image, $popup).attr(
								'src',
								data.path
							);
						} else {
					...
					if (data.aspect_ratio > 0) {
						$(this.defaults.imageWrap, $popup)
							...
							.css({
								'padding-bottom':
									(1 / data.aspect_ratio) * 100 + '%',
							});
					}
					$(this.defaults.imageWrap, $popup).imagesLoaded(
						function () {
							anime({
					...
					$(this.defaults.image, $popup)
						.parent()
						.attr('href', data.url);
					$(this.defaults.error, $popup)
						.empty()
				...
				ifOpened: function ifOpened() {
					return $(this.defaults.popup).hasClass('closed')
						? false
						: true;
				},
				_correctHeight: function _correctHeight() {
					var that = this,
						h = $(that.defaults.popup)
							.find('.container')
							.outerHeight();
					anime({
			...
			function createSelect(el) {
				var SelectModal = {
					...
					_setProduct: function _setProduct(data) {
						...
						if (data) {
							...
							$(that.defaults.title, $popup)
								...
								.attr('href', data.url);
							if (data.path.indexOf('no-image') == -1) {
								$(that.defaults.image, $popup).attr(
									'src',
									data.path
								);
							} else {
							...
							if ($(that.defaults.imageZoom).length) {
								if (
									data.path_image_big.indexOf('no-image') ==
									-1
								) {
								...
								$(that.defaults.imageZoom, $popup).data(
									'caption',
									data.name
								);
							} else
								$(that.defaults.imageLink, $popup).attr(
									'href',
									data.url
								);
							$(that.defaults.error, $popup)
								.empty()
								.html(data.error_message);
							...
							if (
								data.price_old &&
								parseInt(
									data.price_old.replace(/[^0-9]/g, '')
								) >
									parseInt(
										data.price_actual.replace(/[^0-9]/g, '')
									)
							) {
								$(that.defaults.priceOld, $popup)
									.empty()
									.html(data.price_old);
					...
					_handlers: function _handlers() {
						var that = this,
							$popup = $(that.defaults.popup);
						$(that.defaults.options, $popup).on(
							'change',
							function () {
								that._setProduct(
									$('option:selected', this).data('variant')
								);
								that._setFormData(
									$('option:selected', this).data('variant')
								);
								...
								$.fancybox.open(
									...
									],
									{
										...
										beforeShow: function beforeShow() {
											...
											$('.fancybox-container')
												.last()
												.addClass('fancybox--light');
											$body.addClass(
												'fancybox--under-modals'
											);
											if (that.defaults.sticky) {
												$body.addClass(
													'fancybox--from-sticky'
												);
											}
											that.setTimer(60000);
										},
		...
		swatchToggle: function swatchToggle(option) {
			$(option).each(function () {
				var $option = $(this),
					...
					$previewsCarousel = $productblock.find(
					...
					if (!$optionlist.hasClass('js-list-filter')) {
						if (currentSelect) {
							$carousel.find('.slick-slide').each(function (i) {
								if (
									$(this).attr('data-value') == currentSelect
								) {
			...
			var ProductPageGallery = {
				...
				_zoomLinkEvent: function _zoomLinkEvent() {
					...
					if (
						$zoomLink
							.closest('.prd-block')
							.find('.product-previews-carousel').length
					) {
				...
				_handlers: function _handlers() {
					var that = this,
						...
						rows = $previews.closest(that.defaults.doubleSelector)
							.length
							? 2
							: 1,
						slidesToShow =
							$previews.closest(that.defaults.doubleSelector)
								.length &&
							$previews.closest(that.defaults.verticalSelector)
								.length
								? 3
								: $previews.closest(
								...
								? 6
								: 4,
						slidesToShowT = $previews.closest(
							that.defaults.doubleSelector
						).length
							? slidesToShow
							: $previews.closest(that.defaults.verticalSelector)
									.length
					...
					if ($carousel.length) {
						...
						$carousel.on(
							...
							function (event, slick, currentSlide) {
								...
								if (
									slick.closest(that.defaults.doubleSelector)
										.length
								) {
									var previewsRow =
											currentSlide < 2
												? 0
												: Math.floor(currentSlide / 2),
					...
					if ($previews.length) {
						$previews.on('init', function () {
							...
							if (
								!$this.find('.slick-arrow:not(.slick-disabled)')
									.length
							) {
				...
				THEME.productpagegallery_qw.init({
					...
					zoomOptions: {
						...
						zoomContainerAppendTo:
							'.js-product-main-zoom-container',
						zIndex: 149,
						zoomWindowFadeIn: 500,
						zoomWindowFadeOut: 500,
						lensFadeIn: 500,
		...
		actionPrd: function actionPrd() {
			THEME.productAction = {
				...
				clearAllAnimation: function clearAllAnimation() {
					...
					$('.js-minicart-drop-total, .js-hide-empty').addClass(
						'd-none'
					);
	...
	THEME.productPage = {
		...
		progressBar: function progressBar() {
			THEME.progressbar = {
				...
				init: function init(options) {
					...
					$progress.css({
						width:
							(that.data.stockLeft * 100) / that.data.stockMax +
							'%',
					});
				...
				_stockDown: function _stockDown(
					downinterval,
					stockleft,
					stockmin
				) {
					...
					var timer = setInterval(function () {
						...
						$progress.css({
							width:
								(_stockleft * 100) / that.data.stockMax + '%',
						});
				...
				_startCount: function _startCount(
					downinterval,
					stockleft,
					stockmin
				) {
		...
		stickyAddToCart: function stickyAddToCart() {
			THEME.stickyaddtocart = {
				...
				_scrollEvents: function _scrollEvents(
					$stickyAddToCart,
					$button
				) {
					var st = $window.scrollTop();
		...
		productTitleReposition: function productTitleReposition() {
			THEME.prdrepos = {
				init: function init(options) {
					...
					if (
						$(this.default.mobile).closest(
							'.prd-block--mobile-image-first'
						).length
					)
						return false;
			...
			THEME.paymentnotification = {
				...
				init: function init(options) {
					...
					var $paymentNotification = $(
						that.defaults.paymentNotification
					);
					...
					setTimeout(function () {
						that._showPaymentNotification(
							Math.floor(
								Math.random() *
									that.defaults.productArray.length
							)
						);
					}, delay);
				...
				_setData: function _setData(i) {
					...
					$('.js-pn-from').html(
						that.defaults.fromArray[
							random(0, that.defaults.fromArray.length)
						]
					);
					$('.js-pn-time').html(
	...
	THEME.catalog = {
		init: function init() {
			...
			if ($('.js-circle-loader').length)
				this.loadMoreAjax('.js-circle-loader');
		},
		loadMoreAjax: function loadMoreAjax(button) {
			function updateCircleLoader($button) {
				var start =
						($button.data('loaded') * 100) / $button.data('total'),
					to =
						(($button.data('loaded') + $button.data('load')) *
							100) /
						$button.data('total');
				$button.attr('data-start', start);
				$button.attr('data-to', to);
				$('.js-circle-loader-start', $button).html(
					$button.data('loaded')
				);
			...
			$(button).on('click', function (e) {
				...
				$.getJSON($this.attr('href'), function (data) {
					...
					$this.data(
						'loaded',
						$this.data('loaded') + $this.data('load')
					);
					...
					THEME.setPersentCircle
						? THEME.setPersentCircle.reinit()
						: false;
					THEME.loaderHorizontalSm
						? THEME.loaderHorizontalSm.close()
						: false;
				});
				e.preventDefault();
			});
			...
			THEME.setPersentCircle = {
				...
				_handlers: function _handlers() {
					...
					$document
						.on(
							'mouseenter',
							this.defaults.loaderWrap,
							function () {
								var $this = $(this);
								$(that.defaults.bar).removeClass(
									'off-animation'
								);
					...
					$pagination.find('li').on('mouseenter', function () {
						...
						$('.js-pagination-hover-overlay').css({
				...
				reinit: function reinit() {
					...
					if ($(that.defaults.pagination).length)
						that.setStartPage(0);
				},
		...
		sortOptions: function sortOptions(obj) {
			$(obj).each(function () {
				...
				$('li', $sizeList).each(function (i) {
					...
					$this.data(
						'position',
						sortedArray.indexOf($this.data('value'))
					);
				});
			...
			THEME.emptycategorydisable = {
				...
				reinit: function reinit() {
					if ($(this.defaults.emptyBlock).length) {
						$(this.defaults.filterRow).addClass(
							this.defaults.disableClass
						);
					} else {
						$(this.defaults.filterRow).removeClass(
							this.defaults.disableClass
						);
					}
				},
			...
			if ($(el).length) {
				THEME.mobileFilter = {
					...
					setShift: function setShift(delay) {
						var that = this,
							left = $(that.options.mobileFilter).hasClass(
								'filter-col--opened'
							)
								? $(that.options.mobileFilter).offset().left
								: $(that.options.grid).offset();
						$(that.options.mobileFilter).css({
							left: -left + 'px',
						});
						if (isMobile) {
						...
						setTimeout(function () {
							$(that.options.mobileFilter).addClass(
								'filter-col--init'
							);
						}, delay);
					},
						...
						$document.on(
							...
							function (e) {
								if ($(e.target).hasClass('active')) {
									if (isMobile)
										bodyScrollLock.enableBodyScroll();
									$mobileFilter.removeClass(
										'filter-col--opened'
									);
									$body.removeClass(
										'is-fixed is-filters-opened'
									);
									if (!isMobile) {
										setTimeout(function () {
											THEME.product.productWidth('.prd');
			...
			if ($(el).length) {
				THEME.mobileFilter = {
					options: {
						...
						toggleFilter:
							'.js-filter-toggle-horizontal, .js-filter-toggle',
						toggleButton: '.js-filter-btn',
						grid: '.js-category-grid',
					},
					init: function init(options) {
				...
				_handlers: function _handlers() {
					$horGrid.on('click', function (e) {
						...
						if (!$this.is('.active')) {
							...
							$(
								'.list-options [data-toggle="tooltip"]',
								$products
							)
								.attr('data-placement', 'left')
								.tooltip('dispose');
							$(
								'.prd-circle-labels [data-toggle="tooltip"]',
								$products
							)
	...
	THEME.forms = {
		...
		checkoutDisable: function checkoutDisable() {
			$(document).on('change', '[data-checkbox-btn]', function (e) {
				var $this = $(e.target);
				$this.is(':checked')
					? $($this.attr('data-checkbox-btn')).removeClass(
							'btn-disable'
					  )
					: $($this.attr('data-checkbox-btn')).addClass(
							'btn-disable'
					  );
			});
		},
			...
			$contactForm.validate({
				...
				messages: {
					username: {
						...
						minlength:
							'نام کاربری شما می بایست حداقل شامل 2 کاراکتر باشد',
					},
					email: 'لطفا یک آدرس ایمیل معتبر وارد کنید',
					message: 'لطفا یک پیام بنویسید',
				},
		...
		checkoutAccordion: function checkoutAccordion() {
			$('.step-next-accordion').on('click', function () {
				...
				if ($nextPanel)
					$nextPanel.find('.panel-title > a').trigger('click');
				return false;
			});
		},
		...
		productTabs: function productTabs() {
			$('.js-circle-loader').on('click', function (e) {
				...
				$.getJSON($this.attr('href'), function (data) {
					...
					$('.js-circle-loader-start', $this).html(
						$this.data('loaded')
					);
					$('.js-circle-loader-end', $this).html($this.data('total'));

					var start =
							($this.data('loaded') * 100) / $this.data('total'),
						to =
							(($this.data('loaded') + $this.data('load')) *
								100) /
							$this.data('total');
			...
			$('[data-grid-tab-title]').on('click', function (e) {
				...
				THEME.loaderHorizontalSm
					...
					: false;
				THEME.loaderTab
					? THEME.loaderTab.open('productsGrid01')
					: false;
				if ($this.data('loaded') < $this.data('total')) {
				...
				var $loader = $this
						.closest('.holder')
						.find('.js-circle-loader'),
					start = ($this.data('loaded') * 100) / $this.data('total'),
					to =
						(($this.data('loaded') +
							$('.js-circle-loader').data('load')) *
							100) /
						$this.data('total');
				...
				$('.js-circle-loader-start', $loader).html(
					$this.data('loaded')
				);
				...
				$.getJSON($this.attr('href'), function (data) {
					...
					$container.html(html);
					THEME.loaderTab
						? THEME.loaderTab.close('productsGrid01')
						: false;
					THEME.loaderHorizontalSm
						? THEME.loaderHorizontalSm.close()
						: false;
			...
			$('.js-title-tabs').each(function () {
				...
				if ($this.find('.active').length) {
					$this
						.find('.active > [data-grid-tab-title]')
						.trigger('click');
				} else {
					$this
						.find('[data-grid-tab-title]')
						.first()
						.trigger('click');
				}
			});
		...
		creativeSlider: function creativeSlider(el) {
			...
			$slider.on('init', function (slick) {
				...
				setTimeout(function () {
					...
					doAnimations(
						$currentSlide.find('.js-creative-info'),
						'fadeIn',
						1000
					);
					doAnimations(
			...
			$slider.on('afterChange', function (e, slick) {
				...
				var $currentSlide = slick.find('.slick-current'),
					index =
						parseInt($currentSlide.attr('data-slick-index'), 10) +
						1;
				doAnimations(
					$currentSlide.find('.col:not(.prd-single-col)'),
					'fadeInRight',
				...
				$('.js-prd-carousel-vert-2').each(function () {
					...
					var arrowsplace = $this
						...
						? $this
								.parent()
								.parent()
								.find('.js-prd-carousel-vert-arrows')
			...
			THEME.prdcarousel = {
				...
				reinit: function reinit() {
					if (
						...
						if (
							$(this.defaults.carousel).hasClass(
								'slick-initialized'
							)
						) {
						...
						$(this.defaults.carousel).hasClass('slick-initialized')
					) {
						return false;
					}
					$(this.defaults.carousel).each(function () {
						...
						if ($this.parent().find('.carousel-arrows').length) {
							...
							$this.closest('.holder').find('.carousel-arrows')
								.length
						) {
							arrowsplace = $this
								.closest('.holder')
						...
						$(that.defaults.gallery).each(function () {
							...
							if ($filtersList.length) {
								if (w >= maxSM) {
									...
									!$(that.defaults.gallery).data('isotope')
								) {
							...
							$gallery.imagesLoaded(function () {
								$gallery.isotope({
									...
									filter: function filter() {
										var filterResult = that.defaults
											.currentFilter
											? $(this).is(
													that.defaults.currentFilter
											  )
											: true;
										return filterResult;
									},
		...
		carouselTab: function carouselTab() {
			THEME.carouseltab = {
				...
				_handler: function _handler() {
					var that = this;
					if (
						$(that.defaults.carousel).hasClass('.slick-initialized')
					) {
						return false;
					}
					$(
						that.defaults.tab,
						$(that.defaults.carousel).closest('.holder')
					).on('click', function (e) {
						var $this = $(this),
							$carousel = $(
								'#' + $this.parent().attr('data-grid')
							),
	...
	THEME.documentResize = {
		init: function init() {
			...
			function allResize() {
				...
				THEME.slidertexttopshift
					? THEME.slidertexttopshift.reinit()
					: false;
				THEME.setfullheight ? THEME.setfullheight.reinit() : false;
				THEME.setfullheightslider
					? THEME.setfullheightslider.reinit()
					: false;
				...
				if ($('.brand-grid').length)
					THEME.initialization.brandsShowMore();
				$body.removeClass('is-resizing');
			}
			if ((window.innerWidth || $window.width()) != w) {
				THEME.carouseltab ? THEME.carouseltab.hide() : false;
				THEME.flowtype
					? THEME.flowtype.hide('.bnr[data-fontratio]')
					: false;
				THEME.viewMode ? THEME.viewMode.reinit() : false;
			}
			resizeTimer = setTimeout(function () {
				if ((window.innerWidth || $window.width()) == w) {
					...
					THEME.mobilemenupush
						? THEME.mobilemenupush.reinit()
						: false;
					THEME.flowtype
						? THEME.flowtype.reinit('.bnr[data-fontratio]')
						: false;
					THEME.productpagegallery
						? THEME.productpagegallery.reinit()
						: false;
